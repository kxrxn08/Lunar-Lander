...
class DQN:
    ...
    def __init__(self, gamma=0.99, input_size=8, fc_1_dim=512, fc_2_dim=256, out=4, max_size=2500000):
        """
        Description
        -------------------------
        ...
        self.q_network = DQNetwork(
            input_size=input_size, fc_1_dim=fc_1_dim, fc_2_dim=fc_2_dim, out=out)
        self.q_network_target = DQNetwork(
            input_size=input_size, fc_1_dim=fc_1_dim, fc_2_dim=fc_2_dim, out=out)
    ...
    def test2(self, env, n_episodes=1):
        for episode in range(n_episodes):
            state, _ = env.reset()
            done = False
            R = 0
            ...
            print('Test : %d, length : %d, reward : %.3F' %
                  (episode, n_steps, R))
        ...
        for episode in range(n_episodes):
            ...
            while not done:
                ...
                epsilon = epsilon_stop + \
                    (epsilon_start - epsilon_stop)*np.exp(-decay_rate*it)
                next_state, reward, terminated, truncated, _ = env.step(action)
                R += reward
                done = (terminated or truncated)
                self.buffer.add(state, action, reward, next_state, 1 - done)
            ...
            if episode % 1 == 0:
                count += 1
                print('Episode : %d, epsilon : %.3f, length : %d, return : %.3F, Current average : %.3f' % (episode, epsilon, n_steps, R,R_mean))
                episode += 1
            if R_mean > thresh:
    ...
    def test(self, env, n_episodes=10):
        ...
        for episode in range(n_episodes):
            ...
            print('Episode : %d, length : %d, reward : %.3F' %
                  (episode, n_steps, R))
    ...
    def save_gif(self, env, file_name='lunar-lander.gif'):
        ...
        frames[0].save(file_name, save_all=True,
                       append_images=frames[1:], optimize=True, duration=40, loop=0)
    ...
    def save_plot(self,file_save):
        plt.plot(e,m)
        plt.savefig(fname=file_save)